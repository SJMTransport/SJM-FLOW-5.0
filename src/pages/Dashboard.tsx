...
import {
  ...
  UserCircle, Warning, Info, Funnel,
...
export const Dashboard = ({ jurnal, so, coa, piutang, armada = [], sopir = [], armadaDokumen = [], currentUser, onSOClick, onJurnalClick }: any) => {
  ...
  const actionItems = [
    ...
    { count: 5, label: "Invoice jatuh tempo minggu ini", icon: <Info size={16} weight="fill" />, iconColor: "#2563EB", iconBg: "#DBEAFE", border: "#2563EB", bg: "#EFF6FF", action: () => navigate("/invoice") },
  ];
    ...
    <div style={{ display: "flex", height: "calc(100vh - 56px)", overflow: "hidden", background: "#F5F4F1" }}>
      ...
      <div style={{ flex: 1, minWidth: 0, overflowY: "auto", padding: 24 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <span style={sectionLabel}>Ringkasan Operasional</span>
        ...
        {isFinance && (
          <div style={{ marginBottom: 20 }}>
            <span style={sectionLabel}>Ringkasan Keuangan</span>
            <div style={{ display: "flex", gap: 12 }}>
              {[
                ...
                { icon: <Receipt size={28} weight="fill" />, iconBg: "#FEE2E2", iconColor: "#DC2626", value: "372", label: "Invoice Belum Lunas", to: "/invoice", isRp: false },
        ...
        <div style={{ background: "white", border: "1px solid #E2DDD6", borderRadius: 12, overflow: "hidden" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
              <colgroup>
                <col style={{ width: 120 }} /><col style={{ width: 105 }} /><col style={{ width: 165 }} />
                <col style={{ width: 155 }} /><col style={{ width: 155 }} /><col style={{ width: 130 }} />
                <col style={{ width: 100 }} /><col style={{ width: 110 }} /><col style={{ width: 36 }} />
              </colgroup>
              <thead style={{ background: "#F8F6F3" }}>
                <tr>
                  {["ORDER", "TGL MUAT ↕", "CUSTOMER", "RUTE", "SOPIR & ARMADA", "STATUS", "DURASI", "NILAI", ""].map((h, i) => (
                    <th key={i} style={{ textAlign: i === 7 ? "right" : "left", padding: "10px 12px", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", color: "#9B9690", borderBottom: "1px solid #E2DDD6" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {shipmentPaged.length === 0 ? (
                  ...
                  return (
                    <tr key={s.id} onClick={() => onSOClick?.(s.order_id)}
                      ...
                      <td style={tdStyle}>
                        <button onClick={(e) => { e.stopPropagation(); onSOClick?.(s.order_id); }} style={{ fontSize: 12, fontWeight: 700, color: "#EB5E28", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "monospace" }}>{s.order_id}</button>
                      </td>
                      <td style={tdStyle}>{fmtTglMuat(s.tgl_muat)}</td>
                      <td style={{ ...tdStyle, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.customer || "—"}</td>
                      <td style={tdStyle}>
                        <div style={{ fontSize: 13 }}>{s.lokasi_muat || "—"}</div>
                        <div style={{ fontSize: 11, color: "#9B9690", margin: "1px 0" }}>↓</div>
                        <div style={{ fontSize: 13 }}>{s.lokasi_bongkar || "—"}</div>
                      </td>
                      <td style={tdStyle}>
                        <div style={{ fontWeight: 500 }}>{s.nama_sopir || "—"}</div>
                        <div style={{ fontSize: 11, color: "#52504A" }}>{s.no_polisi || ""}{s.jenis_truk ? ` · ${s.jenis_truk}` : ""}</div>
                      </td>
                      <td style={tdStyle}>
                        <span style={{ display: "inline-flex", padding: "3px 10px", borderRadius: 999, fontSize: 12, fontWeight: 500, background: sc.bg, color: sc.color }}>{s.status_muatan}</span>
                      </td>
                      <td style={{ ...tdStyle, color: "#52504A" }}>{["On Going", "Loading", "Completed"].includes(s.status_muatan) ? calcDurasi(s) : "—"}</td>
                      <td style={{ ...tdStyle, textAlign: "right", fontWeight: 600 }}>Rp {fmt(Number(s.total_harga_pajak || s.total_harga || s.harga_pengiriman || 0))}</td>
                      <td style={{ ...tdStyle, textAlign: "center", color: "#9B9690" }}>⋮</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div style={{ padding: "10px 16px", borderTop: "1px solid #E2DDD6", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, color: "#52504A" }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              ...
              {pageNums.map((p, i) => p === "..." ? <span key={i} style={{ padding: "0 4px", color: "#9B9690" }}>…</span> : <button key={p} onClick={() => setShipmentPage(p as number)} style={pagBtnStyle(false, shipmentPage === p)}>{p}</button>)}
      ...
      <div style={{ width: 300, flexShrink: 0, borderLeft: "1px solid #E2DDD6", background: "#FAFAF8", overflowY: "auto", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "20px 16px 16px", borderBottom: "1px solid #E2DDD6" }}>
          <span style={panelLabel}>Dispatcher Hari Ini</span>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
              ...
              <div key={i} style={{ background: "white", border: "1px solid #E2DDD6", borderRadius: 10, padding: "12px 12px 10px", display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ fontSize: 26, fontWeight: 700, color: "#1A1A1A", lineHeight: 1 }}>{d.value}</div>
        ...
        <div style={{ padding: 16, borderBottom: "1px solid #E2DDD6" }}>
          <span style={panelLabel}>Aktivitas Terbaru</span>
          {recentActivity.length === 0 ? <div style={{ padding: "16px 0", textAlign: "center", fontSize: 13, color: "#9B9690" }}>Belum ada aktivitas</div> : (
            recentActivity.map((l: any, i: number) => {
              const sc = STATUS_COLORS[l.status] || { bg: "#F3F4F6", color: "#6B7280" };
              return (
                <div key={i} style={{ display: "flex", gap: 10, padding: "9px 0", borderBottom: i < 4 ? "1px solid #F0EBE4" : "none", alignItems: "flex-start" }}>
                  <div style={{ width: 30, height: 30, borderRadius: "50%", background: sc.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2 }}><ClipboardText size={15} weight="fill" color={sc.color} /></div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      <button onClick={() => onSOClick?.(l.order_id)} style={{ color: "#EB5E28", border: "none", background: "none", fontWeight: 700, cursor: "pointer", padding: 0, fontSize: 12 }}>{l.order_id}</button>
                      {l.location && <span style={{ color: "#52504A", fontWeight: 400 }}> · {l.location}</span>}
                    </div>
                    <div style={{ fontSize: 11, color: "#9B9690" }}>{l.customer || "oleh Operator"}</div>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4 }}>
                    <span style={{ fontSize: 11, color: "#9B9690" }}>{l.time || ""}</span>
                    <span style={{ padding: "2px 7px", borderRadius: 999, fontSize: 10, fontWeight: 500, background: sc.bg, color: sc.color }}>{l.status}</span>
                  </div>
                </div>
              )
            })
          )}
          <div onClick={() => navigate("/activity")} style={viewAllBtn} onMouseEnter={e => e.currentTarget.style.background = "#FEF0E8"} onMouseLeave={e => e.currentTarget.style.background = "#F0EEE9"}>
            <span>Lihat semua aktivitas</span><CaretRight size={14} />
          </div>
        </div>

        <div style={{ padding: "16px 16px 20px" }}>
          <span style={panelLabel}>Action Center</span>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {actionItems.map((a, i) => (
              <div key={i} onClick={a.action} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 9, cursor: "pointer", borderLeft: `3px solid ${a.border}`, background: a.bg }} onMouseEnter={e => e.currentTarget.style.opacity = "0.82"} onMouseLeave={e => e.currentTarget.style.opacity = "1"}>
                ...
                <div style={{ fontSize: 20, fontWeight: 700, minWidth: 30 }}>{a.count}</div>
                <div style={{ flex: 1, fontSize: 12, color: "#52504A", lineHeight: 1.35 }}>{a.label}</div>
                <CaretRight size={14} color="#9B9690" />
              </div>
            ))}
          </div>
        </div>
...
/* ─── Shared Styles ─── */
const sectionLabel: React.CSSProperties = { fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.7px", color: "#6B6760", marginBottom: 12, display: "block" };
const panelLabel: React.CSSProperties = { fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.7px", color: "#6B6760", marginBottom: 12, display: "block" };
const kpiCard: React.CSSProperties = { background: "white", border: "1px solid #E2DDD6", borderRadius: 12, padding: "18px 20px", display: "flex", alignItems: "center", gap: 16, cursor: "pointer", transition: "all 150ms", flex: 1 };
const tdStyle: React.CSSProperties = { padding: "10px 12px", fontSize: 13, color: "#1A1A1A", borderBottom: "1px solid #F0EBE4", verticalAlign: "middle" };
const viewAllBtn: React.CSSProperties = { display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 12px", background: "#F0EEE9", borderRadius: 8, marginTop: 10, fontSize: 12, fontWeight: 500, color: "#EB5E28", cursor: "pointer", transition: "background 150ms" };
const pagBtnStyle = (disabled: boolean, active: boolean): React.CSSProperties => ({ height: 28, minWidth: 28, padding: active ? undefined : "0 10px", borderRadius: 6, border: `1px solid ${active ? "#EB5E28" : "#E2DDD6"}`, background: active ? "#EB5E28" : "white", color: active ? "white" : "#1A1A1A", fontWeight: active ? 600 : 400, fontSize: 12, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.4 : 1, display: "flex", alignItems: "center", justifyContent: "center" });
